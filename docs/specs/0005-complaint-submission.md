...
- `docs/specs/0004-authentication.md` (the `useCurrentUser` hook used by reporter submit; the project root middleware that enforces RBAC on `app/(reporter)/*` plus `/api/complaints`; the `BETTER_AUTH_SECRET` referenced from `ANONYMOUS_TOKEN_SECRET` fallback)
